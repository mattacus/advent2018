...
let nextState = (inputState) => {
  ...
  let sumDigits = scoreSum.toString().split('').map(numStr => Number(numStr));
...
console.log(`Current State: ${currentState.scores.join(' ')}, picks: ${currentState.elfPicks}`);
...
let nextTen = currentState.scores.slice(NUM_RECIPES).slice(0,10).join('');
