  ...
  .then(data => {
    ...
    while(currentStep !== lastStep) {
      ...
      for(let i = 0; i < availableSteps.length; i++) {
        if(Instructions.hasCompletedRequirements(availableSteps[i], stepsTaken)) {
          currentStep = availableSteps[i];
          availableSteps = availableSteps.filter(step => step !== currentStep);
          found = true;
          break;
        }
      }
      if(!found) {
        throw new Error(`No available steps found to take after ${currentStep}`)
      }
    }
    // add final step
    stepsTaken.push(currentStep);
